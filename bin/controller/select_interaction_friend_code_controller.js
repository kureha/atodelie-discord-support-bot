...
class SelectInteractionFriendCodeController {
    constructor() {
        this.fc_repo = new friend_code_1.FriendCodeRepository();
        this.fc_history_repo = new friend_code_history_1.FriendCodeHistoryRepository();
    }
    /**
     * search friend code
     * @param interaction
    ...
    search_friend_code(interaction) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                // search master
                const friend_code_list = yield this.fc_repo.get_t_friend_code_from_game_id(discord_common_1.DiscordCommon.get_guild_id_from_guild(interaction.guild), target_game_id);
                ...
                if (cnt_valid_friend_code == 0) {
                    ...
                    // resolve
                    logger_1.logger.warn(`select game for search friend code failed.`);
                    return false;
                }
                else {
                    // reply friend code
                    ...
                    // resolve
                    logger_1.logger.info(`select game for search friend code completed.`);
                    return true;
                }
            }
            catch (err) {
                // send error message
                logger_1.logger.error(`select game for search friend code error.`, err);
    ...
    regist_friend_code(interaction) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const friend_code_list = yield this.fc_repo.get_t_friend_code(interaction.guild.id, interaction.user.id);
                ...
                // send error message
                logger_1.logger.error(`select game for regist friend code error.`, err);
                yield interaction.reply(`${constants.DISCORD_MESSAGE_EXCEPTION} (Error : ${err})`);
                return false;
            }
            logger_1.logger.info(`select game for regist friend code completed.`);
            return true;
        });
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (is_registed_data) {
                    ...
                    let affected_data_cnt = yield this.fc_repo.delete_t_friend_code(interaction.guild.id, interaction.user.id, target_game_id);
                    logger_1.logger.info(`delete completed. count = ${affected_data_cnt}`);
                    if (affected_data_cnt > 0) {
                        // insert to hisotry
                        affected_data_cnt = yield this.fc_history_repo.insert_t_friend_code(friend_code);
                        ...
                        // resolve
                        logger_1.logger.info(`select game for delete friend code completed.`);
                        return true;
                    }
                    else {
                    ...
                    // resolve
                    logger_1.logger.warn(`select game for delete friend code failed.`);
                    return false;
                }
            }
            catch (err) {
                // send error message
                logger_1.logger.error(`select game for delete friend code error.`, err);
                yield interaction.reply(`${constants.DISCORD_MESSAGE_EXCEPTION} (Error : ${err})`);
                return false;
            }
        });
