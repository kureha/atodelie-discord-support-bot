...
var RecruitmentRepository = /** @class */ (function () {
        ...
        return new Promise(function (resolve, reject) {
            db.serialize(function () {
                ...
                db.run(RecruitmentRepository.SQL_CREATE_M_RECRUITMENT, [], (function (err) {
                    if (err) {
                        logger_1.logger.error("sql exception occured when create table. sql = ".concat(RecruitmentRepository.SQL_CREATE_M_RECRUITMENT));
                        reject(err);
                    }
    ...
    RecruitmentRepository.prototype.insert_m_recruitment = function (data) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                ...
                var sql = "".concat(RecruitmentRepository.SQL_INSERT_M_RECRUITMENT);
                logger_1.logger.info("sql = ".concat(sql, ", id = ").concat(data.id, ", token = ").concat(data.token));
    ...
    RecruitmentRepository.prototype.update_m_recruitment = function (data) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // get prepared statement
                var sql = "".concat(RecruitmentRepository.SQL_UPDATE_M_RECRUITMENT, " WHERE [token] = $token and [delete] = false and datetime([limit_time], 'localtime') >= ").concat(sqlite_utils_1.SqliteUtils.get_now_with_extend(constants.DISCORD_RECRUITMENT_EXPIRE_DELAY_MINUTE_SQL), " ");
    ...
    RecruitmentRepository.prototype.delete_m_recruitment = function (token) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // get prepared statement
                var sql = "".concat(RecruitmentRepository.SQL_DELETE_M_RECRUITMENT, " WHERE [token] = $token and [delete] = false and datetime([limit_time], 'localtime') >= ").concat(sqlite_utils_1.SqliteUtils.get_now_with_extend(constants.DISCORD_RECRUITMENT_EXPIRE_DELAY_MINUTE_SQL), " ");
    ...
    RecruitmentRepository.prototype.get_m_recruitment_id = function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // run serialize
                var sql = "".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT_MAX_ID);
                logger_1.logger.info("sql = ".concat(sql));
                db.get(sql, [], (function (err, row) {
                    if (err) {
                        logger_1.logger.error("sql exception occured when create table. sql = ".concat(RecruitmentRepository.SQL_CREATE_M_RECRUITMENT));
                        reject(err);
                    }
                    logger_1.logger.info("selected max m_reqruitement id successed. : result = ".concat(row.id));
                    resolve(row.id);
                }));
            });
            db.close();
    ...
    RecruitmentRepository.prototype.get_m_recruitment_for_follow = function (server_id, from_datetime, to_datetime) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // run serialize
                var sql = "".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT, " WHERE m1.[server_id] = $server_id AND datetime(m1.[limit_time], 'utc') > datetime($from_datetime) AND datetime(m1.[limit_time], 'utc') <= datetime($to_datetime) ORDER BY m1.[limit_time], m1.[id]");
                logger_1.logger.info("sql = ".concat(sql, ", server_id = ").concat(server_id, ", from_time = ").concat(from_datetime.toLocaleString(), ", to_datetime = ").concat(to_datetime.toLocaleString()));
                db.all(sql, {
                    $server_id: server_id,
    ...
    RecruitmentRepository.prototype.get_m_recruitment_token = function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                ...
                logger_1.logger.debug("token : ".concat(token));
                // run serialize
                var sql = "".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT_TOKEN_COUNT, " ");
                logger_1.logger.info("sql = ".concat(sql));
                db.get(sql, {
                    $token: token
                }, (function (err, row) {
                    if (err) {
                        logger_1.logger.error("sql exception occured when select token count. sql = ".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT_TOKEN_COUNT));
                        reject(err);
                    }
                    if (row.count === 0) {
                        logger_1.logger.info("generate unique token id successed. : result = ".concat(token));
                        resolve(token);
                    }
                    else {
    ...
    RecruitmentRepository.prototype.get_m_recruitment = function (token) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // run serialize
                var sql = "".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT, " WHERE m1.[token] = ? and m1.[delete] = false and datetime(m1.[limit_time] , 'localtime') >= ").concat(sqlite_utils_1.SqliteUtils.get_now_with_extend(constants.DISCORD_RECRUITMENT_EXPIRE_DELAY_MINUTE_SQL), " ");
                logger_1.logger.info("sql = ".concat(sql, ", token = ").concat(token));
                db.get(sql, [token], (function (err, row) {
                    if (err) {
                        logger_1.logger.error("select m_recruitment failed. sql = ".concat(sql, ", key = ").concat(token));
                        reject(err);
                    }
                    else if (row === undefined) {
                        logger_1.logger.error("data not found on m_recruitment. sql = ".concat(sql, ", key = ").concat(token));
                        reject("data not found on m_recruitment. sql = ".concat(sql, ", key = ").concat(token));
                    }
                    logger_1.logger.info("selected single m_reqruitement successed. : key = ".concat(token));
    ...
    RecruitmentRepository.prototype.get_m_recruitment_by_message_id = function (message_id, owner_id) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // run serialize
                var sql = "".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT, " WHERE m1.[message_id] = ? and m1.[owner_id] = ? and m1.[delete] = false and datetime(m1.[limit_time] , 'localtime') >= ").concat(sqlite_utils_1.SqliteUtils.get_now_with_extend(constants.DISCORD_RECRUITMENT_EXPIRE_DELAY_MINUTE_SQL), " ");
                logger_1.logger.info("sql = ".concat(sql, ", message_id = ").concat(message_id, ", owner_id = ").concat(owner_id));
                db.get(sql, [message_id, owner_id], (function (err, row) {
                    if (err) {
                        logger_1.logger.error("select m_recruitment failed. sql = ".concat(sql, ", message_id = ").concat(message_id, ", owner_id = ").concat(owner_id));
                        reject(err);
                    }
                    else if (row === undefined || row.length === 0) {
                        logger_1.logger.error("data not found on m_recruitment. sql = ".concat(sql, ", message_id = ").concat(message_id, ", owner_id = ").concat(owner_id));
                        reject("data not found on m_recruitment. sql = ".concat(sql, ", message_id = ").concat(message_id, ", owner_id = ").concat(owner_id));
                    }
                    logger_1.logger.info("selected single m_reqruitement successed. : message_id = ".concat(message_id, ", owner_id = ").concat(owner_id));
                    logger_1.logger.trace(row);
                    resolve(recruitment_1.Recruitment.parse_from_db(row));
                }));
            });
    ...
    RecruitmentRepository.prototype.get_m_recruitment_latests = function (server_id, count) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db.serialize(function () {
                // run serialize
                var sql = "".concat(RecruitmentRepository.SQL_SELECT_M_RECRUITMENT, " WHERE [server_id] = ? AND datetime(m1.[limit_time], 'localtime') > ").concat(sqlite_utils_1.SqliteUtils.get_now_with_extend(constants.DISCORD_RECRUITMENT_EXPIRE_DELAY_MINUTE_SQL), " ORDER BY m1.[limit_time], m1.[id] LIMIT ").concat(count);
                logger_1.logger.info("sql = ".concat(sql, ", token = ").concat(server_id));
                db.all(sql, [server_id], (function (err, rows) {
                    if (err) {
                        logger_1.logger.error("select m_recruitment failed. sql = ".concat(sql, ", key = ").concat(server_id));
                        reject(err);
                    }
                    else if (rows === undefined || rows === null || rows.length === 0) {
                        logger_1.logger.info("data not found on m_recruitment. sql = ".concat(sql, ", key = ").concat(server_id));
                        resolve([]);
                    }
                    else {
                        // return valie list
                        ...
                        logger_1.logger.info("selected latests m_reqruitement successed. : key = ".concat(server_id));
                        logger_1.logger.trace(rows);
