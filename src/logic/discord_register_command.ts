...
export class DiscordRegisterCommand {
     ...
     * @param client_id 
     */
    static async regist_command(client_id: string): Promise<ServerInfo[]> {
        logger.info(`regist command start.`);

        // get server info from database
        const server_info_list: ServerInfo[] = await new ServerInfoRepository().get_m_server_info_all();
        ...
        return new Promise<ServerInfo[]>((resolve, reject) => {
            ...
            server_info_list.forEach((server_info: ServerInfo) => {
                ...
                // regist command
                rest.put(DiscordRegisterCommand.get_url_for_guild_based_command(client_id, guild_id), { body: commands }).then((data: any) => {
     ...
     * @returns 
     */
    static get_url_for_guild_based_command(client_id: string, guild_id: string): `/${string}` {
     ...
     * @returns 
     */
    static get_regist_slash_command_body(command_list: Discord.SlashCommandBuilder[]): Discord.RESTPostAPIChatInputApplicationCommandsJSONBody[] {
     ...
     * @returns 
     */
    static get_slash_command(name: string, description: string): Discord.SlashCommandBuilder {
