...
export class SelectInteractionServerController {
     ...
     * @param interaction
     */
    static async regist_server_master(interaction: Discord.SelectMenuInteraction, is_check_privillege: boolean = true): Promise<boolean> {
        return new Promise<boolean>(async (resolve, reject) => {
            try {
                ...
                try {
                    // try to insert
                    const affected_data_cnt: number = await server_info_repo.insert_m_server_info(server_info);
                    ...
                    const affected_data_cnt: number = await server_info_repo.update_m_server_info(server_info);
                    logger.info(`update m_server_info completed. affected_data_cnt = ${affected_data_cnt}`);
                }

                // get server info
                server_info = await server_info_repo.get_m_server_info(guild.id);
