...
export class SelectInteractionFriendCodeController {
     ...
     * @param interaction 
     */
    async search_friend_code(interaction: Discord.SelectMenuInteraction): Promise<boolean> {
        try {
            // check values
            ...
            // search master
            const friend_code_list = await this.fc_repo.get_t_friend_code_from_game_id(
            ...
            if (cnt_valid_friend_code == 0) {
                ...
                // resolve
                logger.warn(`select game for search friend code failed.`);
                return false;
            } else {
                // reply friend code
                ...
                // resolve
                logger.info(`select game for search friend code completed.`);
                return true;
            }
        } catch (err) {
            // send error message
            logger.error(`select game for search friend code error.`, err);
     ...
     * @param interaction 
     */
    async regist_friend_code(interaction: Discord.SelectMenuInteraction): Promise<boolean> {
        try {
            // check values
            if (interaction.guild == undefined) {
                throw new Error(`Discord interaction guild is undefined.`);
            ...
            const friend_code_list = await this.fc_repo.get_t_friend_code(interaction.guild.id, interaction.user.id);
     ...
     * @param interaction 
     */
    async delete_friend_code(interaction: Discord.SelectMenuInteraction): Promise<boolean> {
        try {
            // check values
            if (interaction.guild == undefined) {
                throw new Error(`Discord interaction guild is undefined.`);
            ...
            if (is_registed_data) {
                ...
                let affected_data_cnt: number = await this.fc_repo.delete_t_friend_code(interaction.guild.id, interaction.user.id, target_game_id);
                ...
                if (affected_data_cnt > 0) {
                    ...
                    // resolve
                    logger.info(`select game for delete friend code completed.`);
                    return true;
                } else {
                ...
                // resolve
                logger.warn(`select game for delete friend code failed.`);
                return false;
            }
        } catch (err) {
            // send error message
            logger.error(`select game for delete friend code error.`, err);
            await interaction.reply(`${constants.DISCORD_MESSAGE_EXCEPTION} (Error : ${err})`);
