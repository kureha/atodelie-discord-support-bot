...
|#|コマンド文|例|アクション|その他サポートされるコマンド文字列|
| --- | --- | --- | --- | --- |
|1|@(bot名) **募集** *時間文字列(4桁数時、または、～時の構文)を含む募集文*|@botくん 募集 2100から～～を開催しますのでご参加ください。|新規募集を作成。<br>誰でも実行可能。|ぼしゅう(平仮名)|
|2|@(募集メッセージに対するメンション) **編集** *時間文字列(4桁数時、または、～時の構文)を含む募集文*|@botくん 変更 都合により22時からになりましたのでご承知おきください。<br>※ #1へのMentionである必要あり。|募集内容を変更。<br>募集作成者のみ実行可能。|変更、へんしゅう(平仮名)、へんこう(平仮名)|
|3|@(募集メッセージに対するメンション) **中止** *時間文字列(4桁数時、または、～時の構文)を含む募集文*|@botくん 中止 都合により中止します。<br>※ #1へのMentionである必要あり。|募集を中止し削除。<br>募集作成者のみ実行可能。|キャンセル、ちゅうし(平仮名)|
|4|@(bot名) **初期設定** @(募集通知先ロール)|@botくん 初期設定 @通知ロール|募集作成時に通知するチャンネル、及び、ロールを設定。<br>.envの「DISCORD_BOT_ADMIN_USER_ID」にユーザID([確認方法](https://support.discord.com/hc/ja/articles/206346498-%E3%83%A6%E3%83%BC%E3%82%B6%E3%83%BC-%E3%82%B5%E3%83%BC%E3%83%90%E3%83%BC-%E3%83%A1%E3%83%83%E3%82%BB%E3%83%BC%E3%82%B8ID%E3%81%AF%E3%81%A9%E3%81%93%E3%81%A7%E8%A6%8B%E3%81%A4%E3%81%91%E3%82%89%E3%82%8C%E3%82%8B-))を設定したユーザのみ実行可能。|無し|
  
上記の文字列は.envファイルの次の設定値で変更可能です。設定はcsv形式で複数定義可能です。
```bash
